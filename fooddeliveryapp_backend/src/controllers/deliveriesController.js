const Model = require("../models/deliveriesModel");
...
exports.getByShipperId = (req, res) => {
  const { shipperId } = req.params;

  Model.getByShipperId(shipperId, (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({
        success: false,
        message: "Lỗi khi lấy lịch sử giao hàng của shipper",
        error: err.message,
      });
    }
